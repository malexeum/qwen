...
from lib.d1_feature_artifacts import SCHEMA_V1, build_d1_feature_artifact
...
def artifact(analysis_id="fixture_A", git_sha="a" * 40):
    return build_d1_feature_artifact(
        ...
        source_identity={**SOURCE, "fixture_id": analysis_id},
...
def test_manifest_is_deterministic_rebuild_and_uses_lf(tmp_path):
    first = artifact("fixture_A")
    second = artifact("fixture_B")
    ...
    assert [entry["analysis_id"] for entry in entries] == ["fixture_A", "fixture_B"]
...
@pytest.mark.parametrize(
    "analysis_id",
    ["../escape", "features/escape", "C:\\escape", ".", "unicode_тест"],
)
