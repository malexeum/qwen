...
def _afterimages(seed: str, left_path: str, right_path: str) -> list[str]:
    ...
    return ['<g id="d2-afterimages" pointer-events="none">', f'<path d="{left_path}" transform="translate({lx:.2f} {ly:.2f})" fill="#263F61" fill-opacity=".10" filter="url(#afterimage-blur)"/>', f'<path d="{right_path}" transform="translate({rx:.2f} {ry:.2f})" fill="#8E5635" fill-opacity=".085" filter="url(#afterimage-blur)"/>', '</g>']
...
def _tendons(seed: str, body: BodySpec, side: str, clip_id: str) -> list[str]:
    ...
    for _ in range(count):
        ...
        out.append(f'<path d="M {x1:.2f} {y1:.2f} Q {body.cx+rng.uniform(-25,25):.2f} {(y1+y2)/2+bend:.2f} {x2:.2f} {y2:.2f}" stroke="{color}" stroke-width="{rng.uniform(2.8,5.8):.2f}" stroke-opacity="{rng.uniform(.045,.085):.4f}"/>')
    return out+['</g>']
...
def _organs(seed: str, body: BodySpec, side: str, clip_id: str) -> list[str]:
    ...
    for i in range(7 if side == "left" else 5):
        ...
        out.append(f'<path d="{path}" fill="{colors[i%4]}" fill-opacity="{rng.uniform(.055,.135):.4f}" filter="url(#{"organ-deep" if i%2 else "organ-soft"})"/>')
    for i in range(2 if side == "left" else 1):
        ...
        out.append(f'<path d="{_blob(seed,cavity,cavity.salt,roughness=.5)}" fill="#010207" fill-opacity="{rng.uniform(.10,.18):.4f}" filter="url(#organ-deep)"/>')
    return out + ['</g>']
...
def render_svg(seed: str = "d2-blues-v1") -> bytes:
    ...
    svg = ['<?xml version="1.0" encoding="UTF-8"?>', f'<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1260" viewBox="{VIEWBOX}" role="img" aria-labelledby="title desc">', f'<metadata>{metadata}</metadata>', '<title id="title">D2 — TWO-BODY BLUES</title>', '<desc id="desc">Two unequal smoky densities exchange pressure around an unresolved contact.</desc>', '<defs>', '<radialGradient id="body-left"><stop offset="0" stop-color="#315E8E" stop-opacity=".80"/><stop offset=".6" stop-color="#182B43" stop-opacity=".56"/><stop offset="1" stop-color="#080B12" stop-opacity=".04"/></radialGradient>', '<radialGradient id="body-right"><stop offset="0" stop-color="#A35B2B" stop-opacity=".66"/><stop offset=".58" stop-color="#44374F" stop-opacity=".48"/><stop offset="1" stop-color="#080B12" stop-opacity=".04"/></radialGradient>', '<linearGradient id="flow-lr"><stop offset="0" stop-color="#315E8E" stop-opacity="0"/><stop offset=".5" stop-color="#5B4FA8"/><stop offset="1" stop-color="#A35B2B" stop-opacity="0"/></linearGradient>', f'<clipPath id="clip-left-body"><path d="{left_path}"/></clipPath>', f'<clipPath id="clip-right-body"><path d="{right_path}"/></clipPath>', '<filter id="glow" x="-55%" y="-55%" width="210%" height="210%"><feGaussianBlur stdDeviation="72"/></filter>', '<filter id="smoke-body" x="-55%" y="-55%" width="210%" height="210%"><feTurbulence type="fractalNoise" baseFrequency=".005 .012" numOctaves="3" seed="19" result="noise"/><feDisplacementMap in="SourceGraphic" in2="noise" scale="18" result="d"/><feGaussianBlur in="d" stdDeviation="22"/></filter>', '<filter id="mist" x="-40%" y="-100%" width="180%" height="300%"><feGaussianBlur stdDeviation="11"/></filter>', '<filter id="organ-soft" x="-60%" y="-80%" width="220%" height="260%"><feGaussianBlur stdDeviation="16"/></filter>', '<filter id="organ-deep" x="-70%" y="-90%" width="240%" height="280%"><feGaussianBlur stdDeviation="26"/></filter>', '<filter id="flow-glow" x="-70%" y="-150%" width="240%" height="400%"><feGaussianBlur stdDeviation="12"/></filter>', '<filter id="counterflow-blur" x="-40%" y="-80%" width="180%" height="260%"><feGaussianBlur stdDeviation="18"/></filter>', '<filter id="afterimage-blur"><feGaussianBlur stdDeviation="34"/></filter>', '<filter id="tendon-blur"><feGaussianBlur stdDeviation="8"/></filter>', '<filter id="memory-blur"><feGaussianBlur stdDeviation="10"/></filter>', '<filter id="coffin-soft"><feGaussianBlur stdDeviation="3"/></filter>', *physical_defs(), '</defs>', '<rect width="1080" height="1260" fill="#03050A"/>', '<g id="d2-artwork">', *_polygons(seed), *_mist(seed), *_coffin_gravity(seed,contact_x), *burial_backdrop_svg(seed,cfg), *_afterimages(seed,left_path,right_path), '<g id="d2-density-bodies">', f'<path d="{left_path}" fill="#315E8E" fill-opacity=".27" filter="url(#glow)"/>', f'<path id="d2-left-body" d="{left_path}" fill="url(#body-left)" fill-opacity=".50" filter="url(#smoke-body)"/>', f'<path d="{right_path}" fill="#A35B2B" fill-opacity=".23" filter="url(#glow)"/>', f'<path id="d2-right-body" d="{right_path}" fill="url(#body-right)" fill-opacity=".50" filter="url(#smoke-body)"/>', '<g id="d2-internal-organs">', *_organs(seed,left,"left","clip-left-body"), *_organs(seed,right,"right","clip-right-body"), *_tendons(seed,left,"left","clip-left-body"), *_tendons(seed,right,"right","clip-right-body"), '</g>', *_call_response(left,right), '</g>', '<g id="d2-contact-seam">', f'<path d="{seam}" fill="none" stroke="#010207" stroke-width="17" stroke-opacity=".72" filter="url(#flow-glow)"/>', f'<path d="{seam}" fill="none" stroke="#7896BA" stroke-width="5" stroke-opacity=".07" filter="url(#flow-glow)"/>', f'<path d="{seam}" fill="none" stroke="#010207" stroke-width="2.7" stroke-opacity=".90" stroke-linecap="round"/>', '</g>', *_contact_flows(seed,points), *_counterflow(seed,left,right), *_blue_note(points), *_aftertone_memory(seed,right), *burial_foreground_svg(seed,cfg), *tactile_void_svg(seed,1080,1080,.42,760), '</g>', '<g id="d2-footer"><rect y="1080" width="1080" height="180" fill="#03050A"/><line x1="84" x2="996" y1="1100" y2="1100" stroke="#737986" stroke-opacity=".26"/><text x="540" y="1150" text-anchor="middle" fill="#D8D8D5" font-family="Arial,sans-serif" font-size="27" letter-spacing="8">TWO-BODY BLUES</text><text x="540" y="1193" text-anchor="middle" fill="#737986" font-family="Arial,sans-serif" font-size="11" letter-spacing="4">DENSITY · COUNTERFLOW · UNRESOLVED CONTACT</text><text x="540" y="1225" text-anchor="middle" fill="#4A505A" font-family="Arial,sans-serif" font-size="9" letter-spacing="2">CONCEPTUAL VISUALIZATION — NOT A SPECTROGRAM</text></g>', '</svg>']
    return ("\n".join(svg)+"\n").encode("utf-8")
