...
def _body_specs(seed: str) -> tuple[BodySpec, BodySpec]:
    rng = random.Random(seed_int(seed, "body-proportions-v14"))
    left = BodySpec(335 + rng.uniform(-11, 11), 602 + rng.uniform(-18, 18), rng.uniform(250, 268), rng.uniform(305, 328), 28, "left-body-v14")
    right = BodySpec(690 + rng.uniform(-10, 10), 515 + rng.uniform(-22, 22), rng.uniform(190, 202), rng.uniform(232, 248), 26, "right-body-v14")
...
def _contact_points(seed: str, left: BodySpec, right: BodySpec) -> list[tuple[float, float]]:
    rng = random.Random(seed_int(seed, "contact-corridor-v14"))
...
def _mist(seed: str) -> list[str]:
    rng = random.Random(seed_int(seed, "mist-v14"))
...
def _polygons(seed: str) -> list[str]:
    rng = random.Random(seed_int(seed, "polygons-v14"))
...
def _organs(seed: str, body: BodySpec, side: str, clip_id: str) -> list[str]:
    rng = random.Random(seed_int(seed, f"{side}-organs-v14"))
    ...
    for i in range(7 if side == "left" else 5):
        ...
        out.append(f'<path d="{path}" fill="{colors[i%4]}" fill-opacity="{rng.uniform(.045,.115):.4f}" filter="url(#{"organ-deep" if i%2 else "organ-soft"})"/>')
    for i in range(2 if side == "left" else 1):
        ...
        out.append(f'<path d="{_blob(seed,cavity,cavity.salt,roughness=.5)}" fill="#010207" fill-opacity="{rng.uniform(.08,.15):.4f}" filter="url(#organ-deep)"/>')
    return out + ['</g>']


def _call_response(left: BodySpec, right: BodySpec) -> list[str]:
    return ['<g id="d2-call-response" fill="none" stroke-linecap="round" filter="url(#organ-soft)">',
        ...
        f'<path d="M {right.cx-right.rx*.55:.2f} {right.cy+right.ry*.14:.2f} Q {right.cx:.2f} {right.cy+right.ry*.34:.2f} {right.cx+right.rx*.48:.2f} {right.cy+right.ry*.08:.2f}" stroke="#B77951" stroke-width="7" stroke-opacity=".07" clip-path="url(#clip-right-body)"/>', '</g>']


def _contact_flows(seed: str, points: list[tuple[float, float]]) -> list[str]:
    rng = random.Random(seed_int(seed, "contact-flows-v14"))
    out = ['<g id="d2-contact-flows" fill="none" stroke-linecap="round">']
    for i, fraction in enumerate((.18,.36,.54,.72,.84)):
        x, y = points[round(fraction*(len(points)-1))]
        span, rise = rng.uniform(38,74), rng.uniform(-20,20)
        if i % 2 == 0:
            path = f"M {x-span:.2f} {y+rise:.2f} C {x-span*.3:.2f} {y-rise:.2f} {x+span*.3:.2f} {y+rise:.2f} {x+span:.2f} {y-rise*.5:.2f}"
            out.append(f'<path data-mode="attract" d="{path}" stroke="url(#flow-lr)" stroke-width="7" stroke-opacity=".10" filter="url(#flow-glow)"/>')
        else:
            for direction, color in ((-1,"#557DA9"),(1,"#9B6549")):
                path = f"M {x:.2f} {y:.2f} C {x+direction*20:.2f} {y+rise:.2f} {x+direction*span*.55:.2f} {y-rise:.2f} {x+direction*span:.2f} {y+rise*.6:.2f}"
                out.append(f'<path data-mode="repel" d="{path}" stroke="{color}" stroke-width="6" stroke-opacity=".09" filter="url(#flow-glow)"/>')
    return out + ['</g>']


def _counterflow(seed: str, left: BodySpec, right: BodySpec) -> list[str]:
    rng = random.Random(seed_int(seed, "counterflow-bands-v14"))
...
def _blue_note(points: list[tuple[float, float]]) -> list[str]:
    x, y = points[round(.62*(len(points)-1))]
    path = f"M {x-92:.2f} {y+25:.2f} C {x-48:.2f} {y-17:.2f} {x-18:.2f} {y+22:.2f} {x+3:.2f} {y-3:.2f} S {x+46:.2f} {y-33:.2f} {x+72:.2f} {y+6:.2f}"
    return ['<g id="d2-blue-note">', f'<path d="{path}" fill="none" stroke="#5B4FA8" stroke-width="9" stroke-opacity=".11" stroke-linecap="round" filter="url(#flow-glow)"/>', f'<ellipse cx="{x+21:.2f}" cy="{y-12:.2f}" rx="23" ry="10" fill="#5B4FA8" fill-opacity=".10" filter="url(#organ-soft)"/>', '</g>']


def base_metadata(seed: str) -> dict[str, Any]:
    left, right = _body_specs(seed)
    return {"poster_id": POSTER_ID, "schema_version": SCHEMA_VERSION, "renderer": {"name": RENDERER_NAME, "version": RENDERER_VERSION}, "seed": seed,
        ...
        "visual_contract": {"metaphor": "two unequal smoky density bodies exchanging pressure in unresolved counterflow", "retained_layers": ["glow","settling_mist","random_polygons","turbulence","tactile_void"], "structural_layers": ["internal_organs","call_response","aligned_contact_seam","contact_flows","counterflow","blue_note"], "warning": "CONCEPTUAL VISUALIZATION — NOT A SPECTROGRAM"}, "palette": PALETTE}
...
def render_svg(seed: str = "d2-blues-v1") -> bytes:
    ...
    svg = ['<?xml version="1.0" encoding="UTF-8"?>', f'<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1260" viewBox="{VIEWBOX}" role="img" aria-labelledby="title desc">', f'<metadata>{metadata}</metadata>', '<title id="title">D2 — TWO-BODY BLUES</title>', '<desc id="desc">Two unequal smoky densities exchange pressure around an unresolved contact.</desc>', '<defs>',
        ...
        f'<clipPath id="clip-left-body"><path d="{left_path}"/></clipPath>', f'<clipPath id="clip-right-body"><path d="{right_path}"/></clipPath>',
        '<filter id="glow" x="-55%" y="-55%" width="210%" height="210%"><feGaussianBlur stdDeviation="72"/></filter>',
        '<filter id="smoke-body" x="-45%" y="-45%" width="190%" height="190%"><feTurbulence type="fractalNoise" baseFrequency=".006 .015" numOctaves="3" seed="19" result="noise"/><feDisplacementMap in="SourceGraphic" in2="noise" scale="14" result="d"/><feGaussianBlur in="d" stdDeviation="7"/></filter>',
        '<filter id="mist" x="-40%" y="-100%" width="180%" height="300%"><feGaussianBlur stdDeviation="11"/></filter>',
        '<filter id="organ-soft" x="-60%" y="-80%" width="220%" height="260%"><feGaussianBlur stdDeviation="16"/></filter>',
        '<filter id="organ-deep" x="-70%" y="-90%" width="240%" height="280%"><feGaussianBlur stdDeviation="26"/></filter>',
        '<filter id="flow-glow" x="-70%" y="-150%" width="240%" height="400%"><feGaussianBlur stdDeviation="12"/></filter>',
        '<filter id="counterflow-blur" x="-40%" y="-80%" width="180%" height="260%"><feGaussianBlur stdDeviation="18"/></filter>', *physical_defs(), '</defs>',
        '<rect width="1080" height="1260" fill="#03050A"/>', '<g id="d2-artwork">', *_polygons(seed), *_mist(seed), *burial_backdrop_svg(seed,cfg), '<g id="d2-density-bodies">',
        f'<path d="{left_path}" fill="#315E8E" fill-opacity=".29" filter="url(#glow)"/>', f'<path id="d2-left-body" d="{left_path}" fill="url(#body-left)" fill-opacity=".58" filter="url(#smoke-body)"/>',
        f'<path d="{right_path}" fill="#A35B2B" fill-opacity=".25" filter="url(#glow)"/>', f'<path id="d2-right-body" d="{right_path}" fill="url(#body-right)" fill-opacity=".58" filter="url(#smoke-body)"/>',
        '<g id="d2-internal-organs">', *_organs(seed,left,"left","clip-left-body"), *_organs(seed,right,"right","clip-right-body"), '</g>', *_call_response(left,right), '</g>',
        '<g id="d2-contact-seam">', f'<path d="{seam}" fill="none" stroke="#7896BA" stroke-width="9" stroke-opacity=".08" filter="url(#flow-glow)"/>', f'<path d="{seam}" fill="none" stroke="#010207" stroke-width="2.7" stroke-opacity=".90" stroke-linecap="round"/>', '</g>',
        *_contact_flows(seed,points), *_counterflow(seed,left,right), *_blue_note(points), *burial_foreground_svg(seed,cfg), *tactile_void_svg(seed,1080,1080,.55,1100), '</g>',
        '<g id="d2-footer"><rect y="1080" width="1080" height="180" fill="#03050A"/><line x1="84" x2="996" y1="1100" y2="1100" stroke="#737986" stroke-opacity=".26"/><text x="540" y="1150" text-anchor="middle" fill="#D8D8D5" font-family="Arial,sans-serif" font-size="27" letter-spacing="8">TWO-BODY BLUES</text><text x="540" y="1193" text-anchor="middle" fill="#737986" font-family="Arial,sans-serif" font-size="11" letter-spacing="4">DENSITY · COUNTERFLOW · UNRESOLVED CONTACT</text><text x="540" y="1225" text-anchor="middle" fill="#4A505A" font-family="Arial,sans-serif" font-size="9" letter-spacing="2">CONCEPTUAL VISUALIZATION — NOT A SPECTROGRAM</text></g>', '</svg>']
    return ("\n".join(svg)+"\n").encode("utf-8")
