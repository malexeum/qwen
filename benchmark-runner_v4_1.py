...
MORPHOLOGY_FEATURES = [
    'symmetry_score', 'fractal_dim_proxy', 'basin_entropy', 'density_variation',
    'edge_density', 'entropy_orbit', 'entropy_visit', 'kurt_orbit', 'skew_orbit',
    'std_orbit', 'std_visit',
]
...
def build_summary(
    ...
    for r in rows:
        by_gen_preservation[str(r.get('generator', 'unknown'))].append(
    ...
    for r in rows:
        class_preservation[str(r.get('input_class', 'unknown'))].append(
            1.0 if r.get('transition_type') == 'preserved' else 0.0
        )
...
def generate_research_report(
    ...
    L.append('| Generator | N runs |\n|---|---:|\n')
    for gen, n in sorted(summary.get('n_runs_by_generator', {}).items()):
        L.append(f'| {gen} | {n} |\n')
    L.append('\n')
...
def main() -> None:
    ...
    t1 = time.time()
    morph_features = taxonomy.get('morphology_feature_set', MORPHOLOGY_FEATURES)
    print(f'[INFO] morphology_feature_set n={len(morph_features)}, identity_schema_version={taxonomy.get("identity_schema_version")}', flush=True)
        ...
        with ProcessPoolExecutor(max_workers=workers) as ex:
            ...
            for fut in as_completed(futures):
                ...
                try:
                    ...
                    failed_rows.append({'run_id': job['run_id'], 'generator': job['generator'], 'experiment': job['experiment'],
                                         'input_class': job['input_class'], 'instance_id': job['instance_id'],
                                         'module_name': 'execute_single_job', 'version': versions['generator_version'],
                                         'input_id': job['run_id'], 'failure_stage': 'process_pool', 'error': repr(exc)})
                    continue
    ...
    print(f'[INFO] n_features={len(feature_names)}', flush=True)
    ...
    separability = compute_within_between(identity_rows, morph_features, 'input_class') if identity_rows else \
        {'within_mean': 0.0, 'between_mean': 0.0, 'separability': 0.0, 'within_by_class': {}, 'class_means': {}}
    retrieval, dist_matrix, retrieval_labels = compute_retrieval_metrics(identity_rows, morph_features, 'input_class') if identity_rows else \
        ({'top1_accuracy': 0.0, 'top3_accuracy': 0.0, 'n_queries': 0}, None, [])
    print(f'[Stage 7 done in {format_hms(time.time() - t6)}]', flush=True)

    print(f'[Stage 8/{STAGE_TOTAL}] Bifurcation scan...', flush=True)
    t7 = time.time()
    bifurcations = detect_bifurcation_events(identity_rows, morph_features,
                                              protocol.get('bifurcation_detection', {})) if identity_rows else []
    ...
    summary = build_summary(identity_rows, feature_names, separability, retrieval, transitions, failed_rows, len(bifurcations))
    report = generate_research_report(versions, source_mode, identity_rows, feature_names, summary, separability,
                                       retrieval, transitions, bootstrap_by_gen, bifurcations, sensitivity,
                                       seed_sensitivity, observer_variance)
    print(f'[Stage 9 done in {format_hms(time.time() - t8)}]', flush=True)

    print(f'[Stage 10/{STAGE_TOTAL}] Write artifacts...', flush=True)
    t9 = time.time()
    write_csv_with_header(Path(manifest['run_table_path']), RUN_TABLE_HEADER,
                           [[r.get(h, '') for h in RUN_TABLE_HEADER] for r in identity_rows])
    write_csv_with_header(Path(manifest['timing_profile_path']), TIMING_HEADER,
                           [[r['run_id'], r['generator'], r['experiment'], f"{r['elapsed_sec']:.6f}"] for r in timing_rows])
    write_csv_with_header(Path(manifest['failed_runs_path']), FAILED_HEADER,
                           [[r.get(h, '') for h in FAILED_HEADER] for r in failed_rows])

    feat_header = ['run_id', 'generator', 'input_class', 'output_class', 'transition_type', 'instance_id', 'seed',
                    'noise_level', 'deformation_step'] + feature_names
    write_csv_with_header(Path(manifest['features_raw_path']), feat_header,
                           [[r.get(h, '') for h in feat_header] for r in raw_rows])
    write_csv_with_header(Path(manifest['features_normalized_path']), feat_header,
    ...
    for r in identity_rows:
        ...
        by_transition[r.get('transition_type', 'unresolved')].append(r)

    def agg_rows(groups: Dict[str, List[Dict[str, Any]]], key_name: str) -> Tuple[List[str], List[List[Any]]]:
        header = [key_name, 'n_runs'] + morph_features
        rows_out = []
        for k, grows in sorted(groups.items()):
            row = [k, len(grows)]
            for f in morph_features:
                vals = [r[f] for r in grows if f in r and not np.isnan(r[f])]
                row.append(float(np.mean(vals)) if vals else '')
            rows_out.append(row)
        return header, rows_out

    h, r_ = agg_rows(by_gen, 'generator')
    write_csv_with_header(Path(manifest['aggregate_by_generator_path']), h, r_)
    h, r_ = agg_rows(by_class, 'input_class')
    write_csv_with_header(Path(manifest['aggregate_by_class_path']), h, r_)
    h, r_ = agg_rows(by_transition, 'transition_type')
    write_csv_with_header(Path(manifest['aggregate_by_transition_path']), h, r_)

    bif_rows = [[b['generator'], b['input_class'], b['parameter_name'], b['parameter_value'], b['feature_name'],
                 b['gradient_norm'], b['zscore'], b['confirmed_event']] for b in bifurcations]
    write_csv_with_header(Path(manifest['bifurcation_events_path']),
                           ['generator', 'input_class', 'parameter_name', 'parameter_value', 'feature_name',
                            'gradient_norm', 'zscore', 'confirmed_event'], bif_rows)

    if dist_matrix is not None:
        dist_rows = [[retrieval_labels[i]] + [float(v) for v in dist_matrix[i]] for i in range(dist_matrix.shape[0])]
        write_csv_with_header(Path(manifest['pairwise_distance_matrix_path']),
                               ['input_class'] + [f'd{i}' for i in range(dist_matrix.shape[1])], dist_rows)
    else:
        write_csv_with_header(Path(manifest['pairwise_distance_matrix_path']), ['input_class'], [])

    save_json(Path(manifest['centroids_path']), {'identity_schema_version': versions['identity_schema_version'],
                                                  'centroids': centroids})
    save_json(Path(manifest['summary_json_path']), {
        'config_source': source_mode, 'component_versions': versions,
        'distance_metric': DISTANCE_METRIC_NAME, 'normalization_scheme': NORMALIZATION_SCHEME,
        'summary': summary, 'separability': separability, 'retrieval': retrieval, 'transitions': transitions,
        'sensitivity': sensitivity, 'seed_sensitivity': seed_sensitivity, 'observer_variance': observer_variance,
        'n_bifurcations': len(bifurcations), 'n_failed_runs': len(failed_rows),
    })
    ...
    print(json.dumps({'config_source': source_mode, 'n_jobs': len(jobs), 'n_runs': len(identity_rows),
                       'n_failed_runs': len(failed_rows), 'n_bifurcations': len(bifurcations),
                       'output_dir': str(OUT)}, ensure_ascii=False, indent=2), flush=True)
