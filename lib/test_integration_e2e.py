"""End-to-end интеграционный тест всей цепочки:
    audio_features → build_perceptual_latent → build_visual_composition_plan → execute_plan → PNG
...
from lib.composition.planner import build_visual_composition_plan
from lib.reference_renderer.execute_plan import execute_plan, RenderResult


# ──────────────────────────────────────────────────────────────────────────────
# Фикстуры синтетических признаков для разных стилей
# ──────────────────────────────────────────────────────────────────────────────
...
def _make_features(overrides: dict | None = None) -> dict:
...
def blues_latent():
    return build_perceptual_latent(_make_features())


@pytest.fixture(scope="module")
def blues_plan(blues_latent):
    return build_visual_composition_plan(
        audio_features=_make_features(),
        perceptual_latent=blues_latent,
        audio_hash="e2e_test_blues_0000",
        title="Blues Test Track",
        artist="E2E Artist",
    )
...
class TestPipelineStages:
    ...
    def test_latent_keys(self, blues_latent):
        ...
        assert expected.issubset(set(blues_latent.keys()))

    def test_latent_ranges(self, blues_latent):
    ...
    def test_plan_visual_identity(self, blues_plan):
        """visual_identity содержит palette_id."""
    ...
    def test_render_no_save(self, blues_plan):
        """execute_plan без сохранения — корректный RenderResult."""
    ...
    def test_render_saves_png(self, blues_plan, tmp_path):
        """execute_plan создаёт poster.png > 10 KB."""
        result = execute_plan(blues_plan, output_dir=tmp_path, save_png=True)
        assert result.output_path is not None
        assert result.output_path.exists()
        assert result.output_path.stat().st_size > 10_000, "PNG слишком маленький"
...
class TestMultiStylePipeline:
    ...
    def test_plan_and_render(self, style, overrides):
        ...
        latent  = build_perceptual_latent(features)
        plan    = build_visual_composition_plan(
            audio_features=features,
            perceptual_latent=latent,
            audio_hash=f"e2e_{style}_0000",
...
class TestDeterminism:
    ...
    def test_plan_id_stable(self):
        ...
        latent   = build_perceptual_latent(features)
        p1 = build_visual_composition_plan(
            audio_features=features, perceptual_latent=latent,
            audio_hash="det_hash", title="Det Track", artist="Det Artist",
        )
        p2 = build_visual_composition_plan(
            audio_features=features, perceptual_latent=latent,
            audio_hash="det_hash", title="Det Track", artist="Det Artist",
        )
    ...
    def test_render_layers_count_stable(self):
        """layers_rendered одинаково для одного плана."""
        features = _make_features()
        latent   = build_perceptual_latent(features)
        plan = build_visual_composition_plan(
            audio_features=features, perceptual_latent=latent,
...
class TestEdgeCases:
    ...
    def test_silence_track(self):
        ...
        features = _make_features({"energy": 0.0, "rhythm_density": 0.0,
                                   "brightness": 0.0, "dynamic_range": 0.0})
        latent = build_perceptual_latent(features)
        plan   = build_visual_composition_plan(
            audio_features=features, perceptual_latent=latent,
            audio_hash="silence_hash", title="Silence", artist="None",
        )
        result = execute_plan(plan, save_png=False)
        assert result.width == 1024
    ...
    def test_extreme_bpm(self):
        ...
        features = _make_features({"bpm": 300.0, "energy": 0.35,
                                   "rhythm_density": 0.80, "brightness": 0.30})
        latent = build_perceptual_latent(features)
        plan   = build_visual_composition_plan(
            audio_features=features, perceptual_latent=latent,
            audio_hash="extreme_bpm_hash", title="Extreme BPM", artist="None",
        )
        result = execute_plan(plan, save_png=False)
        assert result.width == 1024
    ...
    def test_png_content_not_black(self, tmp_path):
        ...
        plan     = build_visual_composition_plan(
            audio_features=features, perceptual_latent=latent,
            audio_hash="content_check", title="Content Check", artist="Test",
        )
