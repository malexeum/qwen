...
def _compute_silence_rate(rms: np.ndarray) -> float:
    ...
    Порог задаётся как максимум из:
    - абсолютного технического порога 1e-4;
    - 10 % медианного RMS ненулевых фреймов.

    Метрика безразмерная, диапазон [0, 1].
    """
...
def _compute_harmonic_metrics(
    ...
    """
    Возвращает:
    - harmonic_stability: средняя косинусная близость соседних
      нормализованных chroma-векторов, диапазон [0, 1];
    - harmonic_change_rate_hz: число существенных гармонических смен
      в секунду, единица СИ s^-1 (Hz).

    Измерения основаны на реальном ходе chroma по времени, а не на
    искусственной сегментации трека.
    """
    ...
    similarities = np.sum(
        normalized_chroma[:, 1:] * normalized_chroma[:, :-1],
        axis=0,
    )
    ...
    change_threshold = max(
        0.20,
        float(np.percentile(harmonic_distance, 75)),
    )
...
def _compute_spectral_metrics(
    ...
    """
    Возвращает:
    - spectral_flatness: средняя спектральная плоскостность [0, 1];
    - high_frequency_energy_ratio: доля спектральной мощности выше
      f_c = 4000 Hz, либо 0.45 частоты Найквиста для low-sample-rate audio.

    Оба значения безразмерные, ограничены диапазоном [0, 1].
    """
    if y.size == 0 or sr <= 0:
        return 0.0, 0.0
    ...
    spectral_flatness = float(
        np.clip(_safe_mean(spectral_flatness_frames), 0.0, 1.0)
    )
    ...
    high_frequency_mask = frequencies_hz >= cutoff_hz
    if not np.any(high_frequency_mask):
        return spectral_flatness, 0.0

    total_power = float(np.sum(power))
    high_frequency_power = float(np.sum(power[high_frequency_mask, :]))
    high_frequency_energy_ratio = float(
        np.clip(_safe_ratio(high_frequency_power, total_power), 0.0, 1.0)
    )

    return spectral_flatness, high_frequency_energy_ratio
...
def analyze_audio_file(
    ...
    brightness = float(
        np.clip(
            _safe_ratio(spectral_centroid, nyquist_hz),
            0.0,
            1.0,
        )
    )
    ...
    if chroma.size:
        ...
        if chroma_energy > EPSILON:
            ...
            if chroma.shape[1] > lag_frames:
                similarity = np.sum(
                    chroma[:, :-lag_frames] * chroma[:, lag_frames:],
                    axis=0,
                )
    ...
    for key_name, value in features.items():
        clean_features[key_name] = (
            value
            if key_name in structural_keys
    ...
    return [
        {
            ...
            "start_sec": float(index * section_length_sec),
            "end_sec": float(
                min(duration_sec, (index + 1) * section_length_sec)
            ),
        }
...
def build_simple_events(
    ...
    min_separation_frames = max(1, int(round(0.25 * sr / hop_length)))

    selected_indices: List[int] = []
    for index in peak_indices:
        index_int = int(index)
        if not selected_indices or (
            index_int - selected_indices[-1] >= min_separation_frames
        ):
            selected_indices.append(index_int)
    ...
    return [
        {
            "type": "energy_peak",
            "time_sec": float(index * hop_length / sr),
...
def build_perceptual_latent(features: Dict[str, Any]) -> Dict[str, Any]:
    """
    Строит вектор восприятия и переносит измеренные расширенные признаки.

    Для совместимости с историческими анализами отсутствующие поля получают
    нейтральное значение 0.0. Новые анализы заполняют их измеренными данными.
    """
    ...
    harmonic_change_rate_hz = float(
        features.get("harmonic_change_rate_hz", 0.0)
    )
    spectral_flatness = float(features.get("spectral_flatness", 0.0))
    high_frequency_energy_ratio = float(
        features.get("high_frequency_energy_ratio", 0.0)
    )
    ...
    stability = float(
        np.clip(
            0.55 * harmonic_stability
            + 0.25 * (1.0 - tension)
            + 0.20 * (1.0 - spectral_flatness),
            0.0,
            1.0,
        )
    )
